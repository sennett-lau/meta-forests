...
import numpy as np
import time
from main import meta_forests_on_vlcs, vlcs_load_and_extract_features, meta_forests_on_pacs, pacs_load_and_extract_features
...
def meta_forests_hyperparameter_search_on_vlcs():
    ...
    param_grid = {
        ...
        'per_random_forest_max_depth': [3, 5, 10],
    ...
    for params in all_params:
        ...
        try:
            meta_forests_accuracy, _, rf_baseline_accuracy, _, _, _ = meta_forests_on_vlcs(
...
def meta_forests_hyperparameter_search_on_pacs():
    """
    Perform hyperparameter search for the MetaForests model on PACS dataset.
    """
    # Define hyperparameter grid
    param_grid = {
        'epochs': [10, 30],
    ...
    # Initialize or load results dataframe
    results_file = "src/results/pacs_hyperparameter_search_results.csv"
    if os.path.exists(results_file):
        results_df = pd.read_csv(results_file)
        print(f"Loaded existing results file with {len(results_df)} entries")
    else:
    ...
    pacs_domains, training_extracted_features, testing_extracted_features = pacs_load_and_extract_features()
    ...
    for params in all_params:
        ...
        # Train and evaluate model with this parameter combination
        try:
            meta_forests_accuracy, _, rf_baseline_accuracy, _, _, _ = meta_forests_on_pacs(
                epochs=params['epochs'],
                alpha=params['alpha'],
                beta=params['beta'],
                epsilon=params['epsilon'],
...
meta_forests_hyperparameter_search_on_pacs()
